...
char *create_json_obj(struct cloud_data_struct *cloud_data)
{
    ...
    cJSON_AddNumberToObject(cjson_client_controller, "battery", cloud_data->batteryvalue,2);
    ...
    cJSON_AddNumberToObject(cjson_telematic, "time", seconds,0);
...
/*
 * Name : distance_travelled_calculator
 * Description: calculates distance every 1 sec
 * Input parameters: struct cloud_data_struct * : cloud struct
 *                   int speed
 * Output parameters: void
 */
void distance_travelled_calculator(struct cloud_data_struct* cloud_data, int speed)
{
    if (speed > SPEED_THRESHOLD)
    {
        speed = KMPH_TO_MPS_CONVERTER * speed;
        cloud_data->distance_travelled = cloud_data->distance_travelled + (float)(speed * 1);
    }
}
...
void handle_gps_signal_lost(struct cloud_data_struct *cloud_data)
{
    ...
    if((isdigit((int)Lat2)) && (isdigit((int)Lon2))){
