...
int get_gps_data(char *nmea_data, struct gps_data_struct *gps_data)
{
    ...
    if (nmea_data[3] == 'G' && nmea_data[4] == 'G' && nmea_data[5] == 'A')
    {
        ...
        if ((gps_data->latitude >= 90) || (gps_data->latitude <= -90))
        {
        ...
        if ((gps_data->longitude >= 180) || (gps_data->longitude <= -180))
        {
            return GPS_INVALID_QUALITY;
        }
...
void *read_from_gps(void *arg)
{
    ...
    do
    {
        if(can_server == BLE_CAN_MODULE_ID){
            cloud_data->batteryvalue = cloud_data->can_data.battery;
        }
        else 
        {
            cloud_data->batteryvalue = cloud_data->client_controller_data.voltage;
        }
        if (cloud_data->batteryvalue > VOLTAGE_THRESHOLD)  /*Use battery data from CAN in real time or use cloud_data->client_controller_data.voltage in virtual*/
        {
